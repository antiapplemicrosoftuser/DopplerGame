...
public class Main extends Application {
	...
	private final double windowSizeX = 800;
	private final double windowSizeY = 600;
	...
	public void start(final Stage stage) {
		try {
			...
			enemy = new Enemy(400, 400, 30, 300, true);
			bulletList = new ArrayList<Bullet>();
			bulletList.add(new Bullet(400, 400, -200, -200, 10, 200, true));
			...
			Scene scene = new Scene(root, windowSizeX, windowSizeY, Color.WHITE);
	 ...
	 * @param flameTime
	 */
	private void update(double flameTime) {
		player.updatePlayer(mouseX, mouseY, flameTime);
    	enemy.updateEnemy(player, windowSizeX, windowSizeY, flameTime);
    	
    	for (int i = 0; i < bulletList.size(); i++) {
    		bulletList.get(i).updateBullet(flameTime);
    		if (bulletList.get(i).isOutOfCanvas) {
    			bulletList.remove(i);
    		}
    	}
	}
	...
	private void draw() {
		...
		graphicsContext.fillRect(0, 0, windowSizeX, windowSizeY);
		graphicsContext.setFill(Color.BLACK);
		player.drawObject(graphicsContext);
		enemy.drawObject(graphicsContext);
		for (Bullet bullet: bulletList) {
			bullet.drawObject(graphicsContext);
		}
	}
